...
class Section(InheritMixin, BaseModel):
    _INHERIT_CONFIG = {
        'color': ('_color', 'properties.globalSection.color', '#000000'),
        'lineWidth': ('_lineWidth', 'properties.globalSection.lineWidth', 1.0),
    }
    # Core fields
    id: int = Field(default=0)
    time: float = Field(default=0.0)
    text: str = Field(default='Section')
    
    # Private storage for inheritable fields
    _color: Optional[str] = Field(default=None, alias='color', description="Color, None to inherit from globalSection")
    _lineWidth: Optional[float] = Field(default=None, alias='lineWidth', description="Line width, None to inherit from globalSection")
