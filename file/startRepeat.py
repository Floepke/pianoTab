...
class StartRepeat(BaseModel):
    # Core fields
    id: int = Field(default=0)
    time: float = Field(default=0.0)
    
    # Inheritable fields - None means inherit from global properties
    color: Optional[str] = Field(default=None, description="Color, None to inherit from globalStartRepeat")
    lineWidth: Optional[float] = Field(default=None, description="Line width, None to inherit from globalStartRepeat")
    ...
    def _get_inherited_value(self, field_name: str):
        ...
        if field_name == 'color':
            return score.properties.globalStartRepeat.color
        elif field_name == 'lineWidth':
            return score.properties.globalStartRepeat.lineWidth
