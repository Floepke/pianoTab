...
class GridSelector(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.scrollable_frame.grid(row=1, column=0, padx=5, pady=(0, 10), sticky='ew')
    ...
    def set_subdivision(self, value):
        ...
        try:
            ...
            if 1 <= value <= 999:
                ...
                print(f'Warning: Subdivision value {value} out of range (1-999)')
        except (ValueError, TypeError):
            print(f'Warning: Invalid subdivision value: {value}')


# Demo function for testing
def demo_grid_selector():
    '''Demonstrate the GridSelector widget.'''
    
    def on_grid_changed(grid_step):
        print(f'Callback: Grid step changed to {grid_step}')
    ...
    root = ctk.CTk()
    root.title('GridSelector Demo')
    root.geometry('300x500')
    
    # Create GridSelector
    grid_selector = GridSelector(root, on_grid_changed)
    grid_selector.pack(fill='both', expand=True, padx=10, pady=10)
    ...
    def test_get():
        grid_step = grid_selector.get_grid_step()
        grid = grid_selector.get_grid()
        subdivision = grid_selector.get_subdivision()
        print(f'Current: {grid}, subdivision: {subdivision}, grid step: {grid_step}')
    
    def test_set_eighth():
        grid_selector.set_grid('8 - Eighth')
        grid_selector.set_subdivision(3)
    
    test_button1 = ctk.CTkButton(button_frame, text='Get Current', command=test_get)
    test_button1.pack(side='left', padx=5, pady=5)
    
    test_button2 = ctk.CTkButton(button_frame, text='Set Eighth/3', command=test_set_eighth)
    test_button2.pack(side='left', padx=5, pady=5)
